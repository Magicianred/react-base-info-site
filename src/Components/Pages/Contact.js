...
class Contact extends Component {
    ...
    handleSubmit = event => {
        const { firstName, lastName, message, email, errorsData } = this.state;
        const validatedData = !(!errorsData || (errorsData && errorsData.length > 0));
        const txtEmail = `firstName: ${firstName}
lastName: ${lastName}
email: ${email}
...
${message}`;
        ...
        if (validatedData === false) {
            ...
            window.open(`mailto:test@example.com?subject=${encodeURI('from site')}&body=${encodeURI(txtEmail)}`);
            window.location.assign(`${publicUrl}/#/contact/confirm?name=${firstName}&surname=${lastName}`);
        }
    };
                                ...
                                <Row>
                                    <Col>
                                        ...
                                        {errorsData && errorsData.length > 0 ? 
                                            <Badge variant="danger">You have missing data or errors in form</Badge>
                                                : null}
                                    </Col>
                                </Row>
                            </Form>
